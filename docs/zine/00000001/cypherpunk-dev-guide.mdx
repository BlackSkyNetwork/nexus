---
sidebar_position: 2
sidebar_label: 'Cypherpunk Development Principles'
description: 'An open source guide to catalyze the creation of cypherpunk tools'
image: /img/00000001/headers/devguide.jpeg
...
# Cypherpunk Development Principles
<span class="doc-subtitle">An open source guide to catalyze cypherpunk tools</span>

###### By Samadhi, Stellar Magnet, UniPuff
...
Cypherpunks seek to create tools that are "extralegal,” meaning: "not regulated or sanctioned by law". The only real way this construct is possible is with true decentralization while ensuring that a particular tool isn't dependent on the service of a third party, such as a company or a poorly designed "decentralized protocol" that makes compliance decisions on the user’s behalf. For example, regardless of US sanctions being applied to several smart contract addresses, they still manage to exist on the Ethereum blockchain. Similar to how blockchains like Bitcoin or Ethereum can be extralegal, it should be possible for additional applications that are censorship resistant and extralegal like blockchains to exist.

We would like to present an open source guide aimed at developers wishing to create more cypherpunk tools, with best practices that will:
...
The problem is many dApps today fail up against cookie/fingerprinting protections. Fingerprinting is known to be less of a hindrance to applications, but there are cases of dApp's failing to operate without it, such as in Aztec's zk.money V1 frontend. Another example is the Polygon dApp, which fails to function in Firefox without enabling cross-site tracking and fingerprinters. To better understand why this is the case would require a deeper technical analysis. Nevertheless, it is unclear to the user whether they are safe from tracking within this application.
...
IPFS pinning still in some cases requires operating an IPFS node with full uptime so users can access the binary with greater reliability. There are pinning providers like [Pinata](https://www.pinata.cloud/), yet they don't accept crypto payments. You can ask members of your community to help run IPFS nodes and to pin the binaries, or as an alternative, you can look into [Arweave](https://www.arweave.org/), [Filecoin](https://filecoin.io/), or [Swarm](https://ethswarm.org).

### Principle #6: Add a mixnet to offer the best privacy guarantees

The way user's devices interact with the underlying network infrastructure is crucial for privacy and the best guarantee for this is a p2p network with multiple levels of encryption provided by nodes run by participants. For that reason, [Tor](https://blog.torproject.org/arti_100_released/) has been the holy grail of online privacy because of its special multi-layer encryption scheme. "Onion routing," as it is often called, dictates that an information packet from a client to a server must cross three levels of encryption provided by different nodes, one on top of another like an onion. This adds a bit of latency that makes the Tor browser notoriously slower than other browsers at launch, but it provides unrivaled protection of privacy. 

However, Tor has a great vulnerability to sophisticated adversarial parties that have strong statistical software that can monitor the entire network’s ‘entry’ and ‘exit’ nodes, as this can deanonymize users and intercept one's IP address, enabling them to infer patterns of entering and exiting, correlating them back to the user. This issue has yet to be resolved by Tor development.
...
There are a lot of ways to dox yourself and make mistakes. If you’d like to take extra measures on effectively creating an anonymous identity, you can follow this guide we created: [How to Anonymously Build and Contribute to Crypto Projects](https://hackmd.io/Pqyu7FXERaWNA-obOL66Ug).